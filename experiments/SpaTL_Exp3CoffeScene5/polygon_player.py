...
if __name__ == '__main__':
    ...
    specification  = '(F ((mug leftof plate) and (mug ovlp plate)) )'
    specification += 'and ( (not(plate moved plate[-1])) U ((cookies above plate) and (cookies ovlp plate) and (orange above plate) and (orange ovlp plate)) )'
    specification += 'and (F (coffee rightof plate) and (milk closeto coffee) and (milk rightof coffee)) '
    specification +=  'and (F(jug leftof coffee) and (jug closeto coffee))'

    #specification2 = '(F ((mug leftof plate) and (mug ovlp plate)) )'
    specification2 = '( (not(plate moved plate[-1])) U ((cookies above plate) and (cookies ovlp plate) and (orange above plate) and (orange ovlp plate) and (orange leftof cookies)) )'
    #specification2 += 'and (F (coffee rightof plate) and (milk closeto coffee) and (milk rightof coffee)) '
    #specification2 += 'and (F(jug leftof coffee) and (jug closeto coffee))'

    tree = spatial.parse(specification)
    tree2 = spatial.parse(specification2)
    ...
    for i in np.arange(1,len(data)):
        ...
        y2.append(spatial.interpret(tree2, 1, i))
    t1 = time.time()-t0
    ...
    plt.plot(np.array(range(len(y)))/30,y2,linewidth=2)
    plt.plot(len(y)/30,y[-1],'ob',linewidth=2)
    plt.xlabel('Time [s]')
    plt.ylabel('Satisfaction of Specification')
    plt.savefig('exp3-5_until_sat.svg')
    plt.show()

    plot_time_step(1, data, 90)
    plt.savefig('exp3_5_until_t1.svg')
    plt.show()
    plot_time_step(11*30, data, 90)
    plt.savefig('exp3_5_until_t11.svg')
    plt.show()
    plot_time_step(16*30, data, 90)
    plt.savefig('exp3_5_until_t16.svg')
    plt.show()
    plot_time_step(22*30, data, 90)
    plt.savefig('exp3_5_until_t22.svg')
    plt.show()
    plot_time_step(31*30, data, 1)
    plt.savefig('exp3_5_until_t31.svg')
    plt.show()
